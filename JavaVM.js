function createJVM () {
    ...
    function compileByteCodeInstructions (lines) {
        let instructions = [];
                ...
                for (var i = 0; i < arrs.length; i++) {
                    ...
                    if (Number.isNaN(Number(val))) {
                        ...
                        if (opMnemonic.charAt(0) === "#") {
                            ...
                            var indexbyte1 = (opMnemonic >> 8) & 0xFF;
                            var indexbyte2 = opMnemonic & 0xFF;

                            instructions[instructIdx] = indexbyte1;
                            instructions[instructIdx + 1] = indexbyte2;
                        } else {
                            var opcode = opcodes.indexOf(opMnemonic);
    ...
    class JVM_method {
        constructor (lines, parentClass, isConstructor) {
            ...
            this.instructions = compileByteCodeInstructions(lines.slice(instructionsStart, instructionsEnd));
        }
        ...
        run (...args) {
            ...
            function constPoolAt (idx) {
                var splitRef = constantPool[idx].split(" ");

                var refType = splitRef[0];
                var refVal = [];
            ...
            function traceConstPool (arr) {
                return arr.map(a => {
                    if (typeof a === "string" && a.charAt(0) === "#") {
                        let idx = Number(a.slice(1));
                        let ref = constPoolAt(idx);
                        return [ref[0], traceConstPool(ref[1])];
                    } else {
                        return a;
                    }
                });
                ...
                switch (instructions[instructionPointer]) {
                    ...
                    case 182: // invokevirtual
                        ...
                        if (virtualClass === "java/io/PrintStream" && virtualMethodName === "println") {
                            sIdx--;
                            nativeHandlers.println(stack[--sIdx]);
                        }
                    break;

                    case 184: // invokestatic
                        var idx = (instructions[++IP] << 8) | instructions[++IP];
                        var methodName = constantPool[idx].split("//")[1].split(".")[1].split(":")[0];
                        var mthd = findMethod(methodName, classComponents);
                        var mthdRes = mthd.run(...stack);
                        stack[sIdx++] = mthdRes;
                    break;
            ...
            while (IP < instructions.length) {
                var cacheIP = IP + " - " + instructions[IP] + " - " + opcodes[instructions[IP]];
                
                var res = execInstruction(IP);
                IP++;
                
                if (window.location.href.includes("file")) {
                    nativeHandlers.debug("instruction: " + cacheIP + "\nlocals: " + JSON.stringify(locals) + "\nstack: " + JSON.stringify(stack) + "\nresponse: " + res);
                }

                if (typeof res === "object" && res.type === "error") {
                    nativeHandlers.throwError(res.toString(), res.code, res.scope);
                    break;
                }
            }
    ...
    return {
        loadByteCode: function (bytecode) {
            myFile = new JVM_Classfile(bytecode);
            console.log(myFile.class);
        },
        init: function () {
            findMethod("main", myFile.class.components).run();
        },
        attachHandler: function (name, fxn) {
            nativeHandlers[name] = fxn;
        }
    };
}
